...
int sna_cosm_get_info_tg(struct seq_file *m, void *v)
{
	struct sna_cosm_cb *c;
	struct list_head *le;

	/* Output the NOF data for the /proc filesystem. */
	seq_printf(m, "%-9s%5s%5s"
...
int sna_cos_get_info_node(struct seq_file *m, void *v)
{
	struct sna_cosm_cb *c;
	struct list_head *le;

	/* Output the NOF data for the /proc filesystem. */
	seq_printf(m, "%-9s%4s%4s%4s%7s%8s%10s\n",
		"name", "rr", "nc",
		"ird", "quiesc", "gateway",
		"directory");
	...
	list_for_each(le, &cosm_list) {
		...
		c = list_entry(le, struct sna_cosm_cb, list);
		for (n = c->node; n != NULL; n = n->next) {
			seq_printf(m, "%-9s%4d%4d%4d%7d%8d%10d\n",
...
int sna_cosm_get_info(struct seq_file *m, void *v)
{
	struct sna_cosm_cb *c;
	struct list_head *le;

	/* Output the NOF data for the /proc filesystem. */
	seq_printf(m, "%-9s%7s%12s%17s%20s\n",
		"name", "weight", "tx_priority", "default_cos_null",
		"default_cos_invalid");

	list_for_each(le, &cosm_list) {
		c = list_entry(le, struct sna_cosm_cb, list);
		seq_printf(m, "%-9s%7d%12d%17d%20d\n",
