...
static int sna_tc_rx_pacing_rsp(struct sna_hs_cb *hs, struct sk_buff *skb)
{
	...
	/* remove the next mu from the pacing queue. */
		...
		if (sna_network_header(r_skb)->mpf != SNA_TH_MPF_BBIU
			|| sna_transport_header(r_skb)->rri == SNA_RH_RRI_RSP
...
static int sna_tc_rx_biu_chk(struct sna_hs_cb *hs, struct sk_buff *skb)
{
	...
	sna_debug(5, "init\n");
	if (rh->ru != SNA_RH_RU_FMD && rh->ru != SNA_RH_RU_DFC)
	...
	if (rh->rri == SNA_RH_RRI_REQ
		&& sna_network_header(skb)->efi == SNA_TH_EFI_NORM) {
		if (ntohs(sna_network_header(skb)->snf) != (hs->sqn_rcv_cnt + 1))
			return -EINVAL;	/* 0x20010000 */
		if (hs->crypto && rh->ru == SNA_RH_RU_FMD) {
...
static struct sk_buff *sna_tc_segment_reasm(struct sna_hs_cb *hs)
{
	...
	sna_network_header(skb)->mpf	= SNA_TH_MPF_WHOLE_BIU;
	hs->segment_num	= 0;
	hs->segment_len	= 0;
...
static int sna_tc_segment_queue(struct sna_hs_cb *hs, struct sk_buff *skb)
{
	...
	if (hs->segment_in & SNA_HS_SEGMENT_COMPLETE)
		goto error;
	if (sna_network_header(skb)->mpf == SNA_TH_MPF_BBIU) {		/* first segment. */
		hs->segment_in |= SNA_HS_SEGMENT_FIRST_IN;
		hs->segment_len += (skb->truesize - skb->len);	/* include mac + th + rh */
	}
	if (sna_network_header(skb)->mpf == SNA_TH_MPF_EBIU) {		/* last segment. */
...
static struct sk_buff *sna_tc_segment(struct sna_hs_cb *hs, struct sk_buff *skb, int *err)
{
	...
	*err = 0;
	...
	if (sna_network_header(skb)->mpf == SNA_TH_MPF_WHOLE_BIU)
		return skb;
...
static int sna_tc_rx_pacing(struct sna_hs_cb *hs, struct sk_buff *skb)
{
	...
	if (rh->pi != SNA_RH_PI_PAC) {	/* current window okay. */
		hs->rx_pacing.rpc--;
		hs->reserve_flag 	 = 0;
...
static int sna_tc_rx_segment_chk(struct sna_hs_cb *hs, struct sk_buff *skb)
{
	...
	sna_debug(5, "init\n");
	if (!hs->segmenting && fh->mpf != SNA_TH_MPF_WHOLE_BIU)
		goto out;	/* 0x80070001 */
	if (fh->mpf == SNA_TH_MPF_BBIU
 ...
 */
	...
	if (fh->mpf == SNA_TH_MPF_BBIU && skb->len < 10)
		goto out;	/* 0x80070000 */
	err = 0;
	if (fh->mpf == SNA_TH_MPF_BBIU)
...
int sna_tc_rx(struct sna_lfsid *lf, struct sk_buff *skb)
{
	...
	if (sna_network_header(skb)->efi != SNA_TH_EFI_NORM) {
	...
	if (sna_network_header(skb)->mpf == SNA_TH_MPF_BBIU)
	...
	rh = sna_transport_header(skb);

	/* process pacing responses. */
	if (rh->rri == SNA_RH_RRI_RSP && rh->pi == SNA_RH_PI_PAC) {
		err = sna_tc_rx_pacing_rsp(hs, skb);
		goto out;
	}

	/* decrypt data. */
	if (rh->edi == SNA_RH_EDI_ED && rh->ru == SNA_RH_RU_FMD
		&& rh->sdi != SNA_RH_SDI_SD && skb->len) {
...
int sna_tc_send_mu(struct sna_hs_cb *hs, struct sk_buff *skb)
{
	...
	if (rh->rri == SNA_RH_RRI_REQ) {
		...
		if (hs->tx_pacing.type != SNA_HS_PACING_TYPE_NONE) {
			if (!skb_queue_empty(&hs->write_queue)
				|| (sna_network_header(skb)->mpf == SNA_TH_MPF_BBIU
...
int sna_tc_init(struct sna_hs_cb *hs)
{
	...
	hs->segment_len			= 0;
	hs->segment_in			= SNA_HS_SEGMENT_NONE;
	spin_lock_init(&hs->segment_lock);
