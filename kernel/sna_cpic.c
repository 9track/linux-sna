...
static int sna_cpic_fsm_cmcnvi(struct cpic *cpic, int input, int rc,
        int dr, int sr, int fd, int readonly)
{
        sna_debug(5, "init\n");
        return CM_PROGRAM_PARAMETER_CHECK;
...
static int sna_cpic_fsm_cmcnvo(struct cpic *cpic, int input, int rc,
        int dr, int sr, int fd, int readonly)
{
        sna_debug(5, "init\n");
        return CM_PROGRAM_PARAMETER_CHECK;
...
static int sna_cpic_fsm_cmembs(struct cpic *cpic, int input, int rc,
        int dr, int sr, int fd, int readonly)
{
        sna_debug(5, "init\n");
        return CM_PROGRAM_PARAMETER_CHECK;
...
static int sna_cpic_fsm_cmrltp(struct cpic *cpic, int input, int rc,
        int dr, int sr, int fd, int readonly)
{
        sna_debug(5, "init\n");
        return CM_PROGRAM_PARAMETER_CHECK;
...
static int sna_cpic_fsm_cmsltp(struct cpic *cpic, int input, int rc,
        int dr, int sr, int fd, int readonly)
{
        sna_debug(5, "init\n");
        return CM_PROGRAM_PARAMETER_CHECK;
...
static int sna_cpic_fsm_cmwcmp(struct cpic *cpic, int input, int rc,
        int dr, int sr, int fd, int readonly)
{
        sna_debug(5, "init\n");
        return CM_PROGRAM_PARAMETER_CHECK;
	...
	switch (input) {
		...
		case CM_CMCFMD:
			err = sna_cpic_fsm_cmcfmd(cpic, input, rc, dr, sr, fd,
                        ...
                        break;
		case CM_CMCNVI:
			err = sna_cpic_fsm_cmcnvi(cpic, input, rc, dr, sr, fd,
                                readonly);
                        break;
		case CM_CMCNVO:
			err = sna_cpic_fsm_cmcnvo(cpic, input, rc, dr, sr, fd,
                                readonly);
                        break;
		case CM_CMDEAL:
		...
		case CM_CMEID:
			err = sna_cpic_fsm_cmeid(cpic, input, rc, dr, sr, fd,
                        ...
                        break;
		case CM_CMEMBS:
			err = sna_cpic_fsm_cmembs(cpic, input, rc, dr, sr, fd,
                                readonly);
                        break;
		case CM_CMEMN:
		...
		case CM_CMRCVX:
			err = sna_cpic_fsm_cmrcvx(cpic, input, rc, dr, sr, fd,
                        ...
                        break;
		case CM_CMRLTP:
			err = sna_cpic_fsm_cmrltp(cpic, input, rc, dr, sr, fd,
                                readonly);
                        break;
		case CM_CMRTS:
		...
		case CM_CMSLD:
			err = sna_cpic_fsm_cmsld(cpic, input, rc, dr, sr, fd,
                        ...
                        break;
		case CM_CMSLTP:
			err = sna_cpic_fsm_cmsltp(cpic, input, rc, dr, sr, fd,
                                readonly);
                        break;
		case CM_CMSMN:
		...
		case CM_CMWAIT:
			err = sna_cpic_fsm_cmwait(cpic, input, rc, dr, sr, fd,
                        ...
                        break;
		case CM_CMWCMP:
			err = sna_cpic_fsm_cmwcmp(cpic, input, rc, dr, sr, fd,
                                readonly);
                        break;
	}
...
static int sna_cpic_get_index_from_user(unsigned char CM_PTR conversation_index)
{
        int index = 0;
	sna_debug(5, "init\n");
	sna_utok(conversation_index, sizeof(index), &index);
        return index;
}

static int sna_cpic_release_session(struct cpic *cpic)
{
	struct sna_tp_cb *tp = cpic->vi.sna;
	...
	struct cpic **cpic)
{
	...
	if (tp)
		c->vi.sna = tp;
	list_add_tail(&c->list, &cpic_list);
	return err;
}
...
static CM_RETURN_CODE sna_cpic_cmrltp(u_int32_t *tp_name, u_int32_t *tp_name_length)
{
	sna_debug(5, "init\n");
        return CM_CALL_NOT_SUPPORTED;
}

static CM_RETURN_CODE sna_cpic_cmwcmp(void)
{
	sna_debug(5, "init\n");
        return CM_CALL_NOT_SUPPORTED;

#ifdef NOT
unsigned char CM_PTR ooid_list,
        ...
        unsigned char CM_PTR user_field_lisut
#endif
}

static CM_RETURN_CODE sna_cpic_cmembs(__u32 *max_buffer_size)
{
	sna_debug(5, "init\n");
        return CM_CALL_NOT_SUPPORTED;
        ...
        sna_ktou(&size, sizeof(__u32), max_buffer_size);
        return CM_OK;
#endif
}

static CM_RETURN_CODE sna_cpic_cmsltp(__u32 *tp, __u32 *tp_name_length)
{
	sna_debug(5, "init\n");
        return CM_CALL_NOT_SUPPORTED;
}
...
static CM_RETURN_CODE sna_cpic_cmcnvi(unsigned char CM_PTR buffer,
...
static CM_RETURN_CODE sna_cpic_cmcnvo(unsigned char CM_PTR buffer,
        CM_INT32 CM_PTR buffer_length)
{
	sna_debug(5, "init\n");
        return CM_CALL_NOT_SUPPORTED;
...
static CM_RETURN_CODE sna_cpic_cmaccp(unsigned char CM_PTR conversation_id, pid_t *p)
{
	sna_debug(5, "init\n");
        return CM_CALL_NOT_SUPPORTED;
        ...
        if (fd < 0)
                ...
                sna_ktou(&fd, sizeof(int), conversation_id);
 ...
 */
static CM_RETURN_CODE sna_cpic_cminit(unsigned char CM_PTR conversation_id,
        unsigned char CM_PTR sym_dest_name_ptr)
{
	...
	c_fd = cpic_map_fd(cpic);
	if (c_fd < 0) {
		err = CM_PRODUCT_SPECIFIC_ERROR;
		goto out_r;
	}
	err = sna_ktou(&c_fd, sizeof(int), conversation_id);
	if (err < 0) {
		err = CM_PRODUCT_SPECIFIC_ERROR;
                goto out_r;
	}
...
CM_ENTRY sna_cpic_cpicall(unsigned char CM_PTR conversation_id,
        unsigned short opcode, void *uaddr, CM_RETURN_CODE CM_PTR return_code)
{
	...
	switch (opcode) {
		...
		case CM_CMWCMP:
		case CM_CMACCP:
			/* these calls don't have a cpic structure 
			 * allocated yet. 
			 */
			break;
		default:
			cv_index = sna_cpic_get_index_from_user(conversation_id);
			if (!cv_index) {
				sna_debug(5, "bad index\n");
				goto out;
			}
			cpic = cpic_lookup_fd(cv_index, &err);
			if (!cpic || err < 0) {
				sna_debug(5, "unable to locate cpic\n");
				goto out;
			}
			break;
	}
	...
	/* process the command. */
        switch (opcode) {
			...
			cpic_put_fd(cpic);
                        break;
                case CM_CMACCP:
                        rc = sna_cpic_cmaccp(conversation_id, (pid_t *)args->a1);
                        break;
                case CM_CMALLC:
			...
			cpic_put_fd(cpic);
                ...
                case CM_CMCNVI: /* No Cvid */
                        rc = sna_cpic_cmcnvi((__u8 *)args->a1,(CM_INT32 *)args->a2);
                        break;
                case CM_CMCNVO: /* No Cvid */
                        rc = sna_cpic_cmcnvo((__u8 *)args->a1,(CM_INT32 *)args->a2);
                        break;
                case CM_CMDEAL:
		...
		case CM_CMEID:
			...
			cpic_put_fd(cpic);
                ...
                case CM_CMEMBS: /* No Cvid */
                        rc = sna_cpic_cmembs((__u32 *)args->a1);
                        break;
                case CM_CMEMN:
		...
		case CM_CMETPN:
			...
			cpic_put_fd(cpic);
                ...
                case CM_CMINIT: /* No Cvid */
                        rc = sna_cpic_cminit(conversation_id, (__u8 *)args->a1);
                        break;
                case CM_CMPREP:
			...
			rc = sna_cpic_cmrltp((__u32 *)args->a1, (__u32 *)args->a2);
                        break;
                case CM_CMRTS:
			...
			rc = sna_cpic_cmsltp((__u32 *)args->a1, (__u32 *)args->a2);
                        break;
                case CM_CMSMN:
...
out:    sna_cpic_error(return_code, rc);
        return;
}

#ifdef CONFIG_PROC_FS
int sna_cpic_get_info_side(char *buffer, char **start, off_t offset, int length)
{
        struct sna_cpic_side_info *cpic;
        off_t pos = 0, begin = 0;
        struct list_head *le;
...
int sna_cpic_get_info(char *buffer, char **start, off_t offset, int length)
{
	...
	len += sprintf(buffer, "%-9s%-6s%-9s%-8s%-8s\n", "cpicPtr", "state", 
	...
	list_for_each(le, &cpic_list) {
		...
		if (!tp)
			continue;
		len += sprintf(buffer + len, "%p %-6d%-9d%-8d%-8d\n", cpic,
