...
struct sna_lfsid *sna_pc_rx_mu_map_lfsid(struct sk_buff *skb)
{
	...
	sna_debug(5, "init\n");
	return sna_asm_lfsid_get_by_sidhl(fh->odai,
		fh->daf, fh->oaf);
}
...
int sna_pc_rx_mu(struct sk_buff *skb)
{
	...
	switch (sna_transport_header(skb)->ru) {
		/* session traffic. */
		case SNA_RH_RU_FMD:
		case SNA_RH_RU_DFC:
			sna_hs_rx(lf, skb);
...
int sna_pc_get_info(struct seq_file *m, void *v)
{
	struct sna_pc_cb *pc;
	struct list_head *le;

	seq_printf(m, "%-18s%-6s%-10s%-4s%-17s\n",
		"name", "index", "intranode", "tg", "fqpc_id");
	list_for_each(le, &pc_list) {
		pc = list_entry(le, struct sna_pc_cb, list);
		seq_printf(m, "%-18s%-6d%-10d%-4d\n",
