...
int nof_define_class_of_service(int sk, 
	struct sna_define_class_of_service *cos)
{
	int err, size;

        err = setsockopt(sk, SOL_SNA_NOF, SNA_DEFINE_CLASS_OF_SERVICE,
                cos, sizeof(struct sna_define_class_of_service));
        if(err < 0)
        {
...
int nof_delete_class_of_service(int sk, 
	struct sna_delete_class_of_service *dcos)
{
	int err, size;

        err = setsockopt(sk, SOL_SNA_NOF, SNA_DELETE_CLASS_OF_SERVICE,
                dcos, sizeof(struct sna_delete_class_of_service));
        if(err < 0)
        {
...
int nof_start_port(int sk, struct sna_start_port *sp)
{
	int err;
        ...
        if(err < 0)
        {
                perror("nof_start_port setsockopt");
                return (err);
        }
