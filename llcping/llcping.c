/* llcping.c: Linux LLC Ping utility.
...
void pr_pack(char *buf, int cc, struct sockaddr_llc *from)
{
...
int main(int argc, char *argv[])
{
	...
	while ((ch = getopt(argc, argv, "vhfcipqsb")) != EOF)
	{
		switch(ch) {
		case 'b':
			flip_dest = 1;
	...
	to->sllc_test	= 1;
	to->sllc_dsap	= 0;
	to->sllc_ssap	= (unsigned char)LLC_PING_SAP;
	...
	if (argc != 1)
		usage();
	target = *argv;
	in_ether(target, to, 1, flip_dest);
	...
	me.sllc_ssap	= (unsigned char)LLC_PING_SAP;
	...
	printf("LLCPING v%s, LLC PING for Linux.\n", ToolsVersion);
...
static void pinger(void)
{
	...
	(void)gettimeofday((struct timeval *)&outpack[0],
...
{
	(void)fprintf(stderr,
	    "Usage: llcping [-fqvhb] [-c count] [-i wait] [-p pattern]\n\t[-s packetsize] SR:CM:AC:AD:DR:ES DS:TM:AC:AD:DR:ES\n");
